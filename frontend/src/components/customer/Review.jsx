...
export default function Review() {
  ...
  const handleSubmit = async () => {
    ...
    if (!bookingId) { navigate('/customer/receipt'); return; }
    ...
    try {
      ...
      navigate('/customer/receipt', { state: { bookingId } });
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full bg-white p-6 relative overflow-y-auto pb-32">
      <div className="flex flex-col items-center justify-center pt-8">
        ...
        <p className="text-gray-500 font-medium mb-10">How was your experience?</p>
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {TAGS.map(tag => (
                <span
                  ...
                  className={`px-4 py-2 font-bold text-sm rounded-full border cursor-pointer transition-colors
                    ${selectedTags.includes(tag) ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'}`}
                >
                  {tag}
                </span>
              ))}
        ...
        <textarea
          ...
          className="w-full bg-gray-50 border border-gray-200 rounded-2xl p-4 text-sm font-medium text-gray-900 outline-none focus:border-blue-700 h-28 resize-none mb-4"
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-100 flex flex-col gap-3 z-10">
        ...
        <button
          onClick={() => navigate('/customer/receipt', { state: { bookingId } })}
