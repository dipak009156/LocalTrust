...
export default function WorkerChat() {
  ...
  // Fetch booking for customer name + phone
  useEffect(() => {
    if (!bookingId) return;
  ...
  const handleSend = async () => {
    ...
    setMessages(prev => [...prev, {
      id: `opt-${Date.now()}`, senderRole: 'worker', message: text,
    ...
    <div className="flex flex-col h-full bg-[#ece5dd] relative">
      ...
      <div className="bg-[#075e54] text-white px-4 py-3 sticky top-0 z-10 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-sm leading-tight">{customerName}</h1>
            <p className="text-[10px] text-green-300 font-bold">Customer · Active booking</p>
          </div>
        </div>
        {customerPhone && (
          <a href={`tel:${customerPhone}`} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors">
            <Phone size={18} />
          </a>
        )}
      </div>
        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`relative max-w-[78%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                <div
                  className={`px-3 py-2 shadow-sm relative ${isMe ? 'bg-[#dcf8c6]' : 'bg-white'} text-gray-900`}
                  style={{
